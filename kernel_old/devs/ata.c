...
ssize_t ata_read(conn_t *conn, void *buffer, size_t count) {
  ...
  if (conn->offset & 0x01FF) {
    size_t left = 0x0200 - (conn->offset & 0x01FF);

    if (!ata_read_sector(conn, conn->offset >> 9, tmp_buff)) {
      return -1;
    }

    memcpy(buffer, tmp_buff + (conn->offset & 0x01FF), left);
  ...
  for (size_t i = 0; i < count >> 9; i++) {
    if (!ata_read_sector(conn, conn->offset >> 9, buffer)) {
      return -1;
    }
